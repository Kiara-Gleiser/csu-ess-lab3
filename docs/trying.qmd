...
ggplot(state_data, aes(x = date, y = new_cases, fill = state)) +
  ...
  geom_line(aes(y = rolling_mean, color = state), size = 1.2) +  
...
ggplot() +
  ...
  geom_point(data = wmc_data, aes(x = WMC_LON, y = WMC_LAT, color = month, size = total_cases), alpha = 0.7) +
...
deaths_plot <- ggplot() +
  ...
  scale_size_continuous(range = c(1, 10)) +  
  labs(title = "Weighted Mean Center of COVID Deaths",
       x = "Longitude",
       y = "Latitude",
       color = "Month",
       size = "Total Deaths") +
...
cases_plot + deaths_plot + plot_annotation(title = "Weighted Mean Center of COVID Cases and Deaths")
```
